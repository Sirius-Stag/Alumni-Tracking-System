...
router.post(
  ...
  [
    auth,
    [
      check('admission', 'Admission Year is required')
      .not()
      .isEmpty(),
      check('passout', 'PassOut Year is required')
      .not()
      .isEmpty(),
      check('branch', 'College Branch is required')
        .not()
        .isEmpty(),
...
router.get('/', async (req, res) => {
  try {
    const profiles = await Profile.find().populate('user', ['name', 'avatar', 'collegeName']);
    res.json(profiles);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    }).populate('user', ['name', 'avatar', 'collegeName']);
  ...
  [
    ...
    [
      check('projectTitle', 'Project Title is required')
        .not()
        .isEmpty(),
      check('projectField', 'Project Field is required')
        .not()
        .isEmpty(),
      check('projectSkill', 'Project Skill is required')
        .not()
        .isEmpty(),
      check('from', 'From date is required')
        .not()
...
router.delete("/project/:pro_id", auth, async (req, res) => {
  try {
    const foundProfile = await Profile.findOne({ user: req.user.id });
    const proIds = foundProfile.project.map(pro => pro._id.toString());
    // if i dont add .toString() it returns this weird mongoose coreArray and the ids are somehow objects and it still deletes anyway even if you put /education/5
    const removeIndex = proIds.indexOf(req.params.pro_id);
    if (removeIndex === -1) {
      return res.status(500).json({ msg: "Server error" });
    } else {
      // theses console logs helped me figure it out
